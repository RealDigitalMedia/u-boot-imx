...
#define LVDS_MODE IMX_GPIO_NR(4, 6)
...
static struct fb_videomode lvds_fhd = {
	 "FHD", 60, 960, 1080, 7425, 100, 40, 21, 7, 60, 10,
	 FB_SYNC_EXT,
...
static void setup_uart(void)
{
	...
	#if defined CONFIG_MX6DL_DSA2LV
	/* UART2 TXD */
...
//	imx_pwm_config(pwm0, 14705, 50000);	// 1v
//	imx_pwm_config(pwm0, 25000, 50000);	// 1.7v
	imx_pwm_config(pwm0, 29411, 50000);	// 2v
//	imx_pwm_config(pwm0, 36764, 50000);	// 2.5v
//	imx_pwm_config(pwm0, 44117, 50000);	// 3v
	imx_pwm_enable(pwm0);
...
//	}
	...
	if (di == 2) {
		ret = ipuv3_fb_init(&lvds_fhd, 0, IPU_PIX_FMT_RGB24,
				DI_PCLK_LDB, 67000000);
	} else {
		ret = ipuv3_fb_init(&lvds_fhd, di, IPU_PIX_FMT_RGB24,
				DI_PCLK_LDB, 67000000);
	}
...
void panel_info_init(void)
{
	...
	panel_info.vl_col = lvds_fhd.xres;
	panel_info.vl_row = lvds_fhd.yres;
...
void init_dsa2lv_gpio(void) {
	iomux_v3_cfg_t dsa2lv_gpio_pads[] = {
		// GPO
...
//		MX6DL_PAD_KEY_COL2__GPIO_4_10,	//SPI_RESETn

		// GPI
		MX6DL_PAD_SD1_DAT1__GPIO_1_17,	//MLB_ID0
		MX6DL_PAD_SD1_DAT2__GPIO_1_19,	//MLB_ID1
		MX6DL_PAD_SD1_DAT3__GPIO_1_21,	//MLB_ID2
		MX6DL_PAD_SD3_DAT1__GPIO_7_5,	//USB_HPD (uUSB plug in detect)
		MX6DL_PAD_SD3_RST__GPIO_7_8,	//HP_SENSEn (Ear phone plug in detect)
		MX6DL_PAD_ENET_TX_EN__GPIO_1_28,	//PHY_WOL_INT (WOL Interrupt)
//		MX6DL_PAD_ENET_RX_D1__GPIO_1_26,	//RGMII_INT (WOL Interrupt)
		...
		MX6DL_PAD_KEY_COL0__GPIO_4_6,	//LVDS_MODE

		MX6DL_PAD_KEY_ROW0__GPIO_4_7,	//USBIR_STATE_SEL
		MX6DL_PAD_KEY_ROW2__GPIO_4_11,	//SD_VSELECT
		MX6DL_PAD_KEY_ROW4__GPIO_4_15,	//RECOVERY_BUTTON (Android recovery image)
		MX6DL_PAD_GPIO_16__GPIO_7_11,	//PMIC_INTn (Interrupt from PMIC)
		MX6DL_PAD_GPIO_17__GPIO_7_12,	//LVDS_CHANNEL_SEL
		MX6DL_PAD_GPIO_18__GPIO_7_13,	//PWRON_KEY (Power button Interrupt)
		MX6DL_PAD_NANDF_D2__GPIO_2_2,	//SD_Status
		MX6DL_PAD_NANDF_D3__GPIO_2_3,	//SD_WP


//		MX6DL_PAD_ENET_TX_D0__GPIO_1_30,	//RTC_INTn_R (RTC Interrupt)
//		MX6DL_PAD_GPIO_19__GPIO_4_5, 	//AUDAMP_MUTE (COM Ring interrupt)

	};

	// GPIO Init
	// Set multiplexer pin
	mxc_iomux_v3_setup_multiple_pads(dsa2lv_gpio_pads, ARRAY_SIZE(dsa2lv_gpio_pads));

	//setup gpo default
	gpio_direction_output(IMX_GPIO_NR(7, 2), 0);	//Blue LED off
	gpio_direction_output(IMX_GPIO_NR(7, 3), 1);	//Red LED on
	gpio_direction_output(IMX_GPIO_NR(7, 4), 1);	//WIFI disable
	gpio_direction_output(IMX_GPIO_NR(1, 29), 1);	//no eMMC reset
	gpio_direction_output(IMX_GPIO_NR(1, 5), 1);	//no software shutdown
